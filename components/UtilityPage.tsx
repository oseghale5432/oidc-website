...
export default function UtilityPage({ name, eyebrow, headline, introduction, image, imageAlt, facts, sections, noteTitle, note, ctaTitle, ctaText }: UtilityPageProps) {
  return (
      ...
      <main>
          ...
          <div className="mx-auto max-w-[1140px] px-6 lg:px-8">
            <div className="grid gap-12 lg:grid-cols-12 lg:gap-16">
              <div className="lg:col-span-5">
                <p className="text-sm font-semibold text-[#8f6048]">General information</p>
                <h2 className="mt-3 text-3xl font-bold text-[#202124] sm:text-[2.75rem]">{headline}</h2>
              </div>
              <div className="lg:col-span-7">
                <p className="text-[17px] leading-8 text-[#606261]">{introduction}</p>
              </div>
            </div>

            <div className={`mt-14 grid overflow-hidden rounded-[10px] border border-[#dce0e5] ${facts.length === 3 ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
        ...
        <section className="bg-[#f8f8f8] py-20 sm:py-24">
          <div className="mx-auto max-w-[1140px] px-6 lg:px-8">
            <div className="max-w-2xl">
              <p className="text-sm font-semibold text-[#8f6048]">Service overview</p>
              <h2 className="mt-3 text-3xl font-bold text-[#202124] sm:text-[2.75rem]">How the service works</h2>
            </div>
            <div className="mt-10 grid gap-6 lg:grid-cols-3">
              {sections.map((section, index) => (
                <article key={section.title} className="rounded-[10px] border border-[#dce0e5] bg-white p-7">
                  ...
                  {section.points && <ul className="mt-5 space-y-3 border-t border-[#dce0e5] pt-5 text-sm leading-6 text-[#606261]">{section.points.map((point) => <li key={point} className="flex gap-3"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#8f6048]" />{point}</li>)}</ul>}
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 sm:py-24">
          <div className="mx-auto grid max-w-[1140px] gap-10 px-6 lg:grid-cols-[0.7fr_1.3fr] lg:items-start lg:px-8">
            <div><p className="text-sm font-semibold text-[#8f6048]">Service note</p><h2 className="mt-3 text-3xl font-bold text-[#202124]">{noteTitle}</h2></div>
            <div className="rounded-[10px] border border-[#dfcfc3] bg-[#f8f5f2] p-8"><p className="text-[17px] leading-8 text-[#3e403f]">{note}</p></div>
          </div>
        </section>
        ...
        <section className="border-t border-[#dce0e5] py-16">
          <div className="mx-auto flex max-w-[1140px] flex-col gap-6 px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
            <div className="max-w-2xl"><h2 className="text-2xl font-bold text-[#202124] sm:text-3xl">{ctaTitle}</h2><p className="mt-2 text-[#606261]">{ctaText}</p></div>
            <Link href="/contact" className="inline-flex shrink-0 justify-center rounded-[5px] bg-[#202124] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#3e403f]">Contact facilities team</Link>
          </div>
        </section>
      </main>
    </div>
